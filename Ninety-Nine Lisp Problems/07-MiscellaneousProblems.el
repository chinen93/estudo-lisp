...
; jump(N,X/Y,U/V) to express the fact that a knight can jump
; from X/Y to U/V on a NxN chessboard. And finally, represent the solution
; of our problem as a list of N*N knight positions (the knight's tour).
...
; trees, and 14 is already very large, it is extremely difficult to find
; a solution. And remember, we don't know for sure whether there is
; always a solution!
;
...
; a predicate full-words/1 to print (non-negative) integer numbers in
; full words.
...
; bitmap is annotated with the respective lengths of its distinct strings
; of occupied cells. The person who solves the puzzle must complete the
...
; the file p99a.dat, other examples are p99b.dat and p99d.dat. There is
; also an example of a puzzle (p99c.dat) which does not have a solution.
;
...
; the problem, the solution of P28 may be very helpful.
