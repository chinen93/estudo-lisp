...
; sets (nodes and edges), we may use the following Prolog term to represent
; the example graph:
...
; Notice the isolated node d. (Actually, isolated nodes do not even have to
; be atoms in the Prolog sense, they can be compound terms, as
; in d(3.75,blue) instead of d in the example).
;
...
; A small modification of the solution of P83 does the trick. The data of
...
; be a list of nodes that are reachable from this starting point
