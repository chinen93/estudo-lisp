...
; in its right subtree are almost equal, which means their difference is
...
; a given height. The predicate should generate all solutions via backtracking.
...
; levels 1,2,3,...,H-1 contain the maximum number of nodes (i.e 2**(i-1) at
...
; empty successors (the nil's which are not really nodes!) come last.
;
...
; % t(W,X,Y,L,R) represents a (non-empty) binary tree with root W "positioned"
; at (X,Y), and subtrees L and R
;
...
; successor nodes. How tight can you pack together two subtrees to construct
; the combined binary tree?
;
; Use the same conventions as in problem P64 and P65 and test your predicate
; in an appropriate way. Note: This is a difficult problem. Don't give up
; too early!
;
...
; single predicate tree-dlist/2 which does the conversion between a tree
...
; a predicate pre-in-tree/3 that does the job.
;
